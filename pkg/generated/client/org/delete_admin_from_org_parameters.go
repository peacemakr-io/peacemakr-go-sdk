// Code generated by go-swagger; DO NOT EDIT.

package org

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"net/http"
	"time"

	"golang.org/x/net/context"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	cr "github.com/go-openapi/runtime/client"

	strfmt "github.com/go-openapi/strfmt"
)

// NewDeleteAdminFromOrgParams creates a new DeleteAdminFromOrgParams object
// with the default values initialized.
func NewDeleteAdminFromOrgParams() *DeleteAdminFromOrgParams {
	var ()
	return &DeleteAdminFromOrgParams{

		timeout: cr.DefaultTimeout,
	}
}

// NewDeleteAdminFromOrgParamsWithTimeout creates a new DeleteAdminFromOrgParams object
// with the default values initialized, and the ability to set a timeout on a request
func NewDeleteAdminFromOrgParamsWithTimeout(timeout time.Duration) *DeleteAdminFromOrgParams {
	var ()
	return &DeleteAdminFromOrgParams{

		timeout: timeout,
	}
}

// NewDeleteAdminFromOrgParamsWithContext creates a new DeleteAdminFromOrgParams object
// with the default values initialized, and the ability to set a context for a request
func NewDeleteAdminFromOrgParamsWithContext(ctx context.Context) *DeleteAdminFromOrgParams {
	var ()
	return &DeleteAdminFromOrgParams{

		Context: ctx,
	}
}

// NewDeleteAdminFromOrgParamsWithHTTPClient creates a new DeleteAdminFromOrgParams object
// with the default values initialized, and the ability to set a custom HTTPClient for a request
func NewDeleteAdminFromOrgParamsWithHTTPClient(client *http.Client) *DeleteAdminFromOrgParams {
	var ()
	return &DeleteAdminFromOrgParams{
		HTTPClient: client,
	}
}

/*DeleteAdminFromOrgParams contains all the parameters to send to the API endpoint
for the delete admin from org operation typically these are written to a http.Request
*/
type DeleteAdminFromOrgParams struct {

	/*Email*/
	Email string

	timeout    time.Duration
	Context    context.Context
	HTTPClient *http.Client
}

// WithTimeout adds the timeout to the delete admin from org params
func (o *DeleteAdminFromOrgParams) WithTimeout(timeout time.Duration) *DeleteAdminFromOrgParams {
	o.SetTimeout(timeout)
	return o
}

// SetTimeout adds the timeout to the delete admin from org params
func (o *DeleteAdminFromOrgParams) SetTimeout(timeout time.Duration) {
	o.timeout = timeout
}

// WithContext adds the context to the delete admin from org params
func (o *DeleteAdminFromOrgParams) WithContext(ctx context.Context) *DeleteAdminFromOrgParams {
	o.SetContext(ctx)
	return o
}

// SetContext adds the context to the delete admin from org params
func (o *DeleteAdminFromOrgParams) SetContext(ctx context.Context) {
	o.Context = ctx
}

// WithHTTPClient adds the HTTPClient to the delete admin from org params
func (o *DeleteAdminFromOrgParams) WithHTTPClient(client *http.Client) *DeleteAdminFromOrgParams {
	o.SetHTTPClient(client)
	return o
}

// SetHTTPClient adds the HTTPClient to the delete admin from org params
func (o *DeleteAdminFromOrgParams) SetHTTPClient(client *http.Client) {
	o.HTTPClient = client
}

// WithEmail adds the email to the delete admin from org params
func (o *DeleteAdminFromOrgParams) WithEmail(email string) *DeleteAdminFromOrgParams {
	o.SetEmail(email)
	return o
}

// SetEmail adds the email to the delete admin from org params
func (o *DeleteAdminFromOrgParams) SetEmail(email string) {
	o.Email = email
}

// WriteToRequest writes these params to a swagger request
func (o *DeleteAdminFromOrgParams) WriteToRequest(r runtime.ClientRequest, reg strfmt.Registry) error {

	if err := r.SetTimeout(o.timeout); err != nil {
		return err
	}
	var res []error

	// path param email
	if err := r.SetPathParam("email", o.Email); err != nil {
		return err
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}
